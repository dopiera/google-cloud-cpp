...
future<StatusOr<std::pair<bool, Row>>> Table::AsyncReadRow(CompletionQueue&,
                                                           std::string row_key,
                                                           Filter filter) {
  return cq_.WithCompletionQueue(
      [&](CompletionQueue& cq) {
        class AsyncReadRowHandler {
         public:
          ...
          future<bool> OnRow(Row row) {
            ...
            //
            // The `CompletionQueue`, which this object holds a reference to,
            // should not be shut down before `OnStreamFinished` is called. In
            // order to make sure of that, satisying the `promise<>` is deferred
            // until then - the user shouldn't shutown the `CompleetionQue`
            // before this whole operations is done.
